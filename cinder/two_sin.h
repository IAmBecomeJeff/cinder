...
void two_sin(bool old) {                                                              // This is the heart of this_ program. Sure is short.
	if (old) {
		...
		for (int k = 0; k < NUM_LEDS - 1; k++) {
			int old_this_bright = qsuba(cubicwave8((k*all_freq) + this_phase), this_cutoff);     // qsub sets a minimum value called this_cutoff. If < this_cutoff, then bright = 0. Otherwise, bright = 128 (as defined in qsub)..
			int old_thatbright = qsuba(cubicwave8((k*all_freq) + 128 + thatphase), thatcutoff); // This wave is 180 degrees out of phase (with the value of 128).
